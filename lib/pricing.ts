...
const HOURLY_RATE = Number(process.env.PRICING_HOURLY_RATE || 130)
const FEE = Number(process.env.PRICING_PLATFORM_FEE_PCT || 0.2)
const AI_ENABLED = /^(1|true|yes|on)$/i.test(process.env.AI_PRICING_ENABLED || 'false')
...
function heuristic(item: Opportunity) {
  ...
  return pricing(item, { hours_min: hoursMin, hours_max: hoursMax, risk_pct: risk, pricing_note: note, status_manual: status, questions_to_client: defaultQuestions(item), source: 'heuristic' })
}
...
function pricing(item: Opportunity, est: any) {
  const avg=(est.hours_min+est.hours_max)/2
  const netMin=est.hours_min*HOURLY_RATE*(1+Math.max(est.risk_pct-0.1,0))
  const net=avg*HOURLY_RATE*(1+est.risk_pct)
  const netMax=est.hours_max*HOURLY_RATE*(1+est.risk_pct+0.15)
  ...
  const decision_support = {
    ...
    pricing_note: `${est.pricing_note} Cálculo: ${est.hours_min}–${est.hours_max}h × R$ ${HOURLY_RATE}/h + ${Math.round(est.risk_pct*100)}% risco ÷ ${(1-FEE).toFixed(2)}. Líquido alvo: ${brl(roundMoney(net))}.`,
    pricing_calc: { source: est.source, model: est.model, hourly_rate: HOURLY_RATE, platform_fee_pct: FEE, hours_min: est.hours_min, hours_max: est.hours_max, hours_avg: Math.round(avg*10)/10, risk_pct: est.risk_pct, net_target_suggested: roundMoney(net) },
